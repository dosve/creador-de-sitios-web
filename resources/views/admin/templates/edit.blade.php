...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Editar Plantilla: {{ $template->name }}</h3>
        </div>
        
        <form method="POST" action="{{ route('admin.templates.update', $template) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" 
                          ...
                          placeholder="Describe brevemente la plantilla...">{{ old('description', $template->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="html_content" id="html_content" rows="15" 
                          ...
                          placeholder="Ingresa el contenido HTML de la plantilla..." required>{{ old('html_content', $template->html_content) }}</textarea>
                @error('html_content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="css_content" id="css_content" rows="10" 
                          ...
                          placeholder="Ingresa estilos CSS personalizados...">{{ old('css_content', $template->css_content) }}</textarea>
                @error('css_content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="blocks" id="blocks" rows="5" 
                          ...
                          placeholder='[{"type": "text", "content": "Texto de ejemplo"}]'>{{ old('blocks', is_array($template->blocks) ? json_encode($template->blocks, JSON_PRETTY_PRINT) : $template->blocks) }}</textarea>
                @error('blocks')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
