...
<header class="bg-white shadow {{$headerConfig['sticky']??true?'sticky top-0 z-50':''}}">
  <div class="container px-6 py-4 mx-auto">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-violet-600" style="font-family:'Raleway',sans-serif;">{{$website->name??'Academia'}}</h1>
      </div>
      <nav class="hidden lg:flex items-center space-x-6 text-sm font-medium">@include('templates.partials.menu-header')</nav>
      <div class="flex items-center space-x-4"><a href="#cursos" class="hidden md:inline-flex px-6 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700">Explorar Cursos</a><button id="mobile-menu-btn" class="lg:hidden"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
