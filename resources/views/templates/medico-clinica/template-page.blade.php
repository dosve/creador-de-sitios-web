...
<head>
  ...
  <title>{{$page->title??'Página'}} - {{$website->name??'Clínica'}}</title>
...
<body class="bg-cyan-50">@include('templates.medico-clinica.header')<main class="py-20">
    <div class="container px-6 mx-auto max-w-4xl">
      <article class="bg-white p-8 md:p-12 rounded-lg shadow-sm">
        <h1 class="font-heading text-4xl font-bold mb-8 text-cyan-900">{{$page->title??'Título'}}</h1>
        <div class="prose prose-lg max-w-none">{!!$page->html_content??'<p>Contenido...</p>'!!}</div>
      </article>
    </div>
  </main>@include('templates.medico-clinica.footer')</body>
</html>
