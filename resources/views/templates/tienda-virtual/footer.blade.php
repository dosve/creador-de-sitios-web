{{-- Footer para Tienda Virtual --}}
<footer class="py-12 text-white {{ $footerConfig['background'] ?? 'bg-gray-900' }}">
    <div class="container px-4 mx-auto">
        <div class="grid grid-cols-1 gap-8 md:grid-cols-{{ $footerConfig['columns'] ?? 3 }}">
            ...
            <div>
                ...
                @else
                    <h5 class="mb-4 text-xl font-semibold">{{ $website->name ?? 'Mi Tienda' }}</h5>
                @endif
                <p class="text-gray-400">{{ $website->description ?? 'Tu tienda online de confianza' }}</p>
                
                {{-- Redes sociales --}}
                @if($footerConfig['show_social'] ?? true)
                <div class="flex space-x-4 mt-4">
                    @if(!empty($website->facebook_url))
                    <a href="{{ $website->facebook_url }}" target="_blank" class="text-gray-400 hover:text-white">
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                    @endif
                    @if(!empty($website->instagram_url))
                    <a href="{{ $website->instagram_url }}" target="_blank" class="text-gray-400 hover:text-white">
                    ...
                    <a href="{{ $website->twitter_url }}" target="_blank" class="text-gray-400 hover:text-white">
